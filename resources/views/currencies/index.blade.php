...
<div class="container mx-auto px-4 py-6">
    ...
    <nav aria-label="breadcrumb" class="mb-4 flex justify-start">
        <ol class="flex text-gray-500 font-semibold dark:text-white-dark">
            <li>
                <a href="javascript:;" class="hover:text-gray-500/70 dark:hover:text-white-dark/70 dongrek-font">{{ __('currencies.home') }}</a>
            </li>
            <li class="before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4">
                <a href="javascript:;" class="text-primary dongrek-font">{{ __('currencies.currencies') }}</a>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 ">
        <h2 class="text-xl font-semibold mb-2 sm:mb-0 dark:text-white dongrek-font">{{ __('currencies.currencies') }}</h2>
        <button class="btn-green flex items-center" onclick="openCreateModal()">
            <i class="fas fa-plus-circle mr-2"></i>
            <span class="font-semibold dongrek-font">{{ __('currencies.add_new') }}</span>
        </button>
    </div>
    ...
    <div class="mt-6 p-4 bg-white rounded shadow dark:bg-[#1b2e4b]">
        <table class="w-full whitespace-nowrap shadow-sm">
            <thead class="bg-gray-100 dark:bg-[#1b2e4b]" style="color: blue;">
                <tr>
                    <th class="px-4 py-2 dongrek-font">{{ __('currencies.id') }}</th>
                    <th class="px-4 py-2 dongrek-font">{{ __('currencies.code') }}</th>
                    <th class="px-4 py-2 dongrek-font">{{ __('currencies.name') }}</th>
                    <th class="px-4 py-2 dongrek-font">{{ __('currencies.symbol') }}</th>
                    <th class="px-4 py-2 dongrek-font">{{ __('currencies.actions') }}</th>
                </tr>
            </thead>
            <tbody>
...
<div id="modalBackdrop" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-start justify-center transition-opacity duration-300">
    ...
    <div id="currencyModal" class="relative mt-10 w-full max-w-md bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg transform transition-all duration-300 ease-out opacity-0 -translate-y-12">
        <h2 id="modalTitle" class="text-lg font-semibold mb-3 dark:text-white dongrek-font">{{ __('currencies.create_new_currency') }}</h2>
        <form id="currencyForm" method="POST">
            @csrf
            <input type="hidden" name="_method" id="formMethod" value="POST">
            <div class="mb-3">
                <label class="block text-sm font-medium mb-1 dark:text-white dongrek-font">{{ __('currencies.currency_code') }}</label>
            ...
            <div class="mb-3">
                <label class="block text-sm font-medium mb-1 dark:text-white dongrek-font">{{ __('currencies.currency_name') }}</label>
            ...
            <div class="mb-3">
                <label class="block text-sm font-medium mb-1 dark:text-white dongrek-font">{{ __('currencies.symbol') }}</label>
            ...
            <div class="flex justify-end space-x-2 mt-4">
                <button type="button" onclick="closeModal()" class="btn-gray dongrek-font">{{ __('currencies.cancel') }}</button>
                <button type="submit" class="btn-green dongrek-font" id="saveButton">{{ __('currencies.save') }}</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    function openCreateModal() {
        ...
        document.getElementById('modalTitle').innerText = '{{ __('currencies.create_new_currency') }}';
        ...
        document.getElementById('saveButton').innerText = '{{ __('currencies.save') }}';
    }

    function openEditModal(currency) {
        document.getElementById('modalBackdrop').classList.remove('hidden');
        document.getElementById('currencyModal').classList.remove('opacity-0', 'translate-y-[-30px]', 'scale-95');
        document.getElementById('modalTitle').innerText = '{{ __('currencies.edit_currency') }}';
        ...
        document.getElementById('saveButton').innerText = '{{ __('currencies.update') }}';
    }
