    ...
    <nav aria-label="breadcrumb" class="mb-4 flex justify-start">
        <ol class="flex text-gray-500 font-semibold dark:text-white-dark">
            ...
            <li class="before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4">
                <a href="javascript:;" class="text-primary">Monthly Sales Report</a>
            </li>
        </ol>
    </nav>

    <h2 class="text-2xl font-bold text-gray-800 mb-4">Monthly Sales Report</h2>

    <!-- Filter Form -->
    <form action="{{ route('reports.monthly-sales') }}" method="GET" class="mb-4">
        <div class="flex space-x-2">
            <!-- Month Input -->
            <input type="month" name="month" value="{{ request('month', $selectedMonth) }}" class="form-input  h-9">
            <div class="text-center">
                <button type="submit" class="btn-blue px-6 py-2 rounded text-white">Submit</button>
            </div>
            <div class="text-center">
                <a href="{{ route('reports.print_monthlysale', ['month' => request('month', $selectedMonth)]) }}"
        ...
        <table class="w-full border border-gray-200">
            ...
            <tbody class="text-gray-800">
                @foreach($monthlySales as $index => $sale)
                    <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-gray-100' }} border-b hover:bg-gray-200">
                        ...
                        <td class="px-6 py-3">{{ \Carbon\Carbon::parse($sale->sale_month)->format('F Y') }}</td>
            ...
            <tfoot class="text-gray-900 font-semibold">
                <tr>
                    ...
                    <td class="px-6 py-3 text-blue-700">{{ number_format($monthlySales->sum('total_quantity'), 0) }}</td>
                    <td class="px-6 py-3 text-green-700">${{ number_format($monthlySales->sum('total_sales'), 2) }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
    ...
    @if ($monthlySales->total() > 0)
        ...
        <div>
            <select id="per_page" class="input"
                onchange="window.location.href='{{ $monthlySales->url(1) }}&per_page=' + this.value">
        ...
        <div class="pagination">
            ...
            @else
                <a href="{{ $monthlySales->previousPageUrl() }}&per_page={{ request('per_page') }}" class="btn-pagination">←</a>
            @endif

            <span class="btn-page">{{ $monthlySales->currentPage() }}</span>

            @if ($monthlySales->hasMorePages())
                <a href="{{ $monthlySales->nextPageUrl() }}&per_page={{ request('per_page') }}" class="btn-pagination">→</a>
            @else
                <span class="btn-disabled">→</span>
            @endif
        </div>
    @else
        <p class="text-center text-gray-600 dark:text-gray-300">No monthly sales available.</p>
    @endif
</div>
