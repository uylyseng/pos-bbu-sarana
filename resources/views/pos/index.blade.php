...
@section('content')
  ...
  <div class="min-h-screen bg-gray-100 dark:bg-gray-900 flex">
    ...
    <div class="flex justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-md shadow">
      ...
      <a href="{{ route('dashboard.cashier') }}" class="text-blue-500 hover:underline">
    ...
    <div class="mb-2 p-2">
      ...
      <option value="{{ $table->id }}">
        {{ $table->name }} ({{ $table->group->name ?? 'No Group' }})
      </option>
    @endforeach
        </select>
      </div>
      ...
      @endphp
        <option value="{{ $customer->id }}" data-has-coupon="{{ $hasCoupon ? 'true' : 'false' }}"
          data-coupon-discount="{{ $couponDiscount }}">
          {{ $customer->name }}
        </option>
    @endforeach
        </select>
...
<button id="payButton" class="pay-button w-full px-3 py-1 bg-green-500 hover:bg-green-600 text-white rounded-md text-sm h-10" onclick="processPayment()">
  <span id="payButtonText">Pay</span>
    ...
    <div class="mb-4 bg-white dark:bg-gray-700  p-4 rounded-[6px] flex">
      ...
      <div class="flex space-x-2">
        ...
        <!-- <img src="{{ asset('icons/icontable.png') }}" class="w-6 h-6"> -->
        ...
        <!-- <img src="{{ asset('icons/category.png') }}" class="w-7 h-7"> -->
      ...
      rounded-[3px] hover:bg-gray-600 focus:outline-none transition-colors duration-200" title="Shift Details"
        ...
        <button onclick="window.open('{{ route('customerscreen') }}', '_blank');" ,
      ...
      <div class="flex items-center space-x-2">
        ...
        <img class="w-10 h-10 rounded-full" src="{{ Auth::user() && Auth::user()->profile
    ...
    @endforeach
      ...
      @endphp
        <div class="table-card" id="table_{{ $table->id }}" data-status="{{ $table->status }}"
          data-group="{{ $group->id }}" onclick="selectTable({{ $table->id }})"
          style="{{ $isHeld ? 'background-color: red;' : '' }}">
          <img src="{{ asset('assets/images/tables/table.png') }}" alt="Table Image">
          <p>
          <span class="table-name">{{ $table->name }}</span><br>
          <span id="tablePrice_{{ $table->id }}" class="table-total">
          @if($isHeld && isset($table->held_order_total))
        Total: ${{ number_format($table->held_order_total, 2) }}
      @endif
          </span>
          </p>
        </div>
  ...
  <div id="productsGrid"
    ...
    @foreach($categories ?? [] as $category)
      @foreach($category->products ?? [] as $product)
        <div class="product-card flex flex-col items-center bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-md p-3" data-category-id="{{ $category->id }}" onclick="showProductPopup({{ json_encode($product) }})">
          <!-- Image -->
          <img src="{{ $product->image ? asset('storage/' . $product->image) : asset('assets/images/shoping.png') }}"
               alt="{{ $product->name_en }}"
      ...
      <!-- Category Grid -->
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 overflow-y-auto"
              ...
              <div class="category-card dark:bg-gray-700 shadow transition-shadow"
                  ...
                  <img src="{{ $category->image ? asset('storage/' . $category->image) : asset('assets/images/noimages.png') }}"
  ...
  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 overflow-y-auto" style="max-height: calc(150vh - 300px);">
    ...
    <div class="category-card dark:bg-gray-700 shadow transition-shadow cursor-pointer" onclick="selectCategory('all')">
      <img src="{{ asset('assets/images/select-all.png') }}" alt="Show All" class="category-img">
    ...
    @foreach($categories ?? [] as $category)
      <div class="category-card dark:bg-gray-700 shadow transition-shadow cursor-pointer" onclick="selectCategory('{{ $category->id }}')">
        <img src="{{ $category->image ? asset('storage/' . $category->image) : asset('assets/images/shoping.png') }}"
             alt="{{ $category->name }}" class="category-img">
        <div class="category-card-title">{{ $category->name }}</div>
        <div class="category-card-subtitle {{ count($category->products) == 0 ? 'text-red-500 dark:text-red-400' : 'dark:text-gray-300' }}">
          {{ count($category->products) }} Products
        </div>
      </div>
    @endforeach
 ...
 <span class="block w-full border-b border-gray-300 dark:border-gray-600 mb-3"></span>
  ...
  <div class="relative p-4 w-[500px] md:w-[485px] bg-white dark:bg-gray-800 rounded-lg">
    ...
    <form id="closeShiftForm" onsubmit="submitCloseShift(event)">
      ...
      <input type="hidden" id="shiftUserId" value="{{ Auth::user()->id }}" />
      <input type="hidden" id="shiftId" value="{{ $shift->id ?? '' }}" />
      ...
      <div class="flex justify-between mb-4">
        ...
        <div class="w-1/2 pr-2">
          ...
          <div class="mb-4">
            ...
            <span class="text-gray-800 dark:text-gray-200">{{ $shift->closer->name ?? Auth::user()->name }}</span>
          </div>

          <!-- CASH IN HAND (READ-ONLY) -->
          <div class="mb-4">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Cash In Hand:</label>
            <span class=" text-gray-800 dark:text-gray-200">{{ $shift->cash_in_hand ?? '0.00' }}</span>
          </div>
        </div>
        ...
        <div class="w-1/2 pl-2">
          ...
          <div class="mb-4">
            ...
            <span class=" text-gray-800 dark:text-gray-200">{{ $shift->time_open ?? now() }}</span>
          </div>

          <!-- CLOSE AT -->
          <div class="mb-4">
            <label class=" text-sm font-medium text-gray-700 dark:text-gray-300">Close At:</label>
            <span class=" text-gray-800 dark:text-gray-200">{{ now() }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="mb-4">
        ...
        <input type="number" value="{{ $shift->total_cash ?? '0.00' }}" step="0.01" id="cashSubmitted" class="w-full p-2 border rounded dark:bg-gray-700 dark:text-white" placeholder="Enter final cash amount" required />
      </div>
  ...
  >
    ...
    <div class="space-y-3 text-sm text-gray-700 dark:text-gray-300">
      <div>
        ...
        <span id="shiftOpenTime">{{ $shift->time_open ?? now() }}</span>
      </div>
      <hr>
      <div>
        <span class="font-semibold">Open By:</span>
        <span id="shiftOpenBy">{{ $shift->closer->name ?? Auth::user()->name }}</span>
      </div>
      <hr>
      <div>
        <span class="font-semibold">Closed At:</span>
        <span id="shiftCloseTime">{{ now() }}</span>
      </div>
      <hr>
      <div>
        <span class="font-semibold">Closed By:</span>
        <span id="shiftCloseBy">{{ $shift->closer->name ?? Auth::user()->name }}</span>
      </div>
      <hr>
      <div>
        <span class="font-semibold">Cash In Hand:</span>
        <span id="shiftCashInHand">{{ $shift->cash_in_hand ?? '0.00' }}</span>
      </div>
      <hr>
      <div>
        <span class="font-semibold">Cash Total:</span>
        <span id="shiftCashSubmitted">{{ $shift->total_cash ?? '0.00' }}</span>
      </div>
      <hr>
 ...
 function showShiftDetailsModal() {
  ...
  function closeShiftDetailsModal() {
    ...
    try {
      ...
      const createdBy = "{{ Auth::user()->id }}"; // For audit logging
  ...
  try {
    ...
    const response = await fetch("{{ route('pos.updateHold') }}", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content
    ...
    try {
      const response = await fetch("{{ route('shifts.close') }}", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      ...
      window.location.href = "{{ route('shifts.create') }}";
      return;
      }
  ...
  try {
    const response = await fetch("{{ route('payments.updatePayment') }}", {
...
async function holdOrder() {
  ...
  const total = parseFloat(document.getElementById("totalAmount")?.innerText) || 0;
  const totalPeople = document.getElementById("totalPeople")?.value || 0;
  const createdBy = "{{ Auth::user()->id }}"; // current user ID
  ...
  try {
    const response = await fetch("{{ route('pos.holdOrder') }}", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content
...
async function loadHeldOrder(tableId) {
  try {
    const response = await fetch("{{ route('pos.loadHeldOrder') }}?table_id=" + tableId);
    if (!response.ok) {
